...
std::vector<uint8_t> decode_stream(const uint8_t* data, size_t size, const FrameOptions& opts) {
    ...
    while (pos + sizeof(uint32_t) * 3 <= size) {
        ...
        FSETables tables(params);
        // Default to the LSB reader when the writer used LSB, unless explicitly overridden.
        const bool use_lsb_reader = opts.use_lsb_reader || opts.use_lsb;
        auto decoder = make_decoder(opts.level, tables, use_lsb_reader);
